...
public class Hunter extends GameObject {
    ...
    // Üçgenin sadece merkez etrafında dönmesini sağlayacak özel transform objesi
    ...
    public Hunter(double x, double y, int maxHealth, int maxVacuum) {
        ...
        // İlk nokta her zaman (0,0) yani Hunter'ın tam merkezidir.
        scanner = new Polygon(
            0.0, 0.0, 
            scannerRange, -75.0, 
            scannerRange, 75.0
        ...
        // DÖNÜŞ EKSENİ (PIVOT) AYARI:
        // Polygon'un kendi etrafında değil, Hunter'ın merkezinde (0,0) dönmesini sağlar.
        scannerRotation = new Rotate(0, 0, 0);
        scanner.getTransforms().add(scannerRotation);

        // Hunter'ın çarpışmada dikkate alınan ana gövdesi.
        body = new Circle(0, 0, 15, Color.ORANGE);
        
        // Önce scanner eklendiği için Hunter'ın gövdesinin altında kalır.
    ...
    // RangeToken toplandığında scanner üçgenini büyütür.
    public void increaseRange(double amount) {
        scannerRange += amount;
        scanner.getPoints().clear();
        scanner.getPoints().addAll(
            0.0, 0.0,                             // Merkez sabit kalır
            scannerRange, -(scannerRange / 2.0),  // Yeni sağ üst
            scannerRange, (scannerRange / 2.0)    // Yeni sağ alt
        );
    }
    ...
    public void rotateScanner(double targetX, double targetY) {
        // Karakterin merkezi ile fare arasındaki açıyı hesapla (Radyan cinsinden)
        double angle = Math.atan2(targetY - this.y, targetX - this.x);
        
        // Radyanı dereceye çevir
        double degrees = Math.toDegrees(angle);
        
        // Rotate objesinin açısını değiştiriyoruz (Pivot 0,0'da sabit kalıyor)
        scannerRotation.setAngle(degrees);
    }
